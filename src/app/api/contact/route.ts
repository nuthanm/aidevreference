...
import { autoReplyTemplate, contactAdminTemplate } from "@/lib/email-templates";
import { isMailerConfigured, sendMail } from "@/lib/mailer";
import { checkRateLimit, getRequestIp } from "@/lib/rate-limit";
import { verifyTurnstileToken } from "@/lib/turnstile";
import { contactSchema, zodErrorToFieldMap } from "@/lib/validators";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const adminMail = contactAdminTemplate(parsed.data);
    await sendMail({ to: adminTo, subject: adminMail.subject, text: adminMail.text, html: adminMail.html });

    const userMail = autoReplyTemplate(parsed.data.name, parsed.data.subject);
    await sendMail({ to: parsed.data.email, subject: userMail.subject, text: userMail.text, html: userMail.html });

    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json({ ok: false, error: "Unable to submit contact request." }, { status: 500 });
  }
}
